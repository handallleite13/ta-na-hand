...
// 1. Remove the improperly placed regexes
code = code.replace(/const teamTrainingRegex = new RegExp\([^;]+;\n/g, '');
code = code.replace(/const outrosEsportesRegex = [^\n]*\n/g, '');
code = code.replace(/const globalSportsRegex = [^\n]*\n/g, '');

// 2. Add them safely AFTER allOtherSports
const regexesDef = `
...
code = code.replace(/const allOtherSports = new RegExp[^\n]*\n/, (match) => match + regexesDef);
